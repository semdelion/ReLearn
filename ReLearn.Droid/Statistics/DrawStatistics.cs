...
namespace ReLearn.Droid.Statistics
{
    public class DrawStatistics : BaseStatistics
    {
            ...
            float paddingBottom = 15f)
        {
            ...
            var step = (_width - paddingLeftRight * 2) / (@true + @false);

            var y0 = _height - paddingBottom - heightLine;
            var y1 = _height - paddingBottom;
        ...
        {
            Typeface Plain = null;
            var bold = Typeface.Create(Plain, TypefaceStyle.Normal);
            var paint = new Paint
            {
                AntiAlias = true,
                TextSize = fontSize,
