            ...
            try
            {
                using (Bitmap Image1 = Bitmap.CreateBitmap(Width, Width, Bitmap.Config.Argb4444))
                {
                    ...
                    using (Bitmap Image2 = Bitmap.CreateScaledBitmap(image, (int)(Width / 2.5 * 2 - WidthLine), (int)(Width / 2.5 * 2 - WidthLine), false))
                        using (Canvas baseCan = new Canvas(Image1))
                        {
                            FRAME.DrawPieChart(baseCan, API.Statistics.GetAverageNumberLearn(Database_NL_and_D), Settings.StandardNumberOfRepeats, Start, End, (float)(baseCan.Height / 2.5), WidthLine);
                            return CreateSingleImageFromMultipleImages(Image1, Image2, new PointF(((FRAME.Left + FRAME.Width) / 2) - (float)(baseCan.Height / 2.5) + WidthLine / 2, ((FRAME.Top + FRAME.Height) / 2) - (float)(baseCan.Height / 2.5) + WidthLine / 2));
                        }
                }
            }
